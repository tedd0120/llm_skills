...
6. 组织树可视化：抓取完成后自动生成离线单文件 HTML（紧凑布局、iOS 极简风格，支持缩放、拖拽、搜索与高亮）
...
1. 首屏显示根节点，根节点横向排布
2. 点击节点展开或收起直属下级
3. 节点悬停 1 秒显示“根 -> 父级”的上游祖先链（姓名-部门）
4. 节点文本可鼠标框选，文本选择期间不触发画布拖动
5. 姓名搜索命中后自动居中到目标节点；同名结果可通过 `pre` / `next` 按钮循环切换居中
6. 支持滚轮缩放、拖动画布（空白区）与关键字搜索高亮
7. 页面使用 iOS 极简视觉与 Apple 系优先字体栈
