    ...
    def do_POST(self):
        ...
        for p in (req.get("paths") or []):
            ...
            if rp not in allow:
                ...
                return
            # 二级护栏：只允许用户目录或应用安装目录（后者仅 open 用，删除白名单不含它）
            roots = [HOME]
            if sys.platform == "darwin":
                roots.append("/Applications")
            elif sys.platform.startswith("win"):
                roots.append(os.environ.get("ProgramFiles", "C:\\Program Files"))
                roots.append(os.environ.get("ProgramFiles(x86)", "C:\\Program Files (x86)"))
