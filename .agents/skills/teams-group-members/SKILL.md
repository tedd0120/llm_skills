...
6. 组织树可视化：抓取完成后自动生成离线单文件 HTML（支持分栏钻取、常驻路径条、全局搜索跳转与底部字段条）
...
HTML 页面特性：
1. 分栏逐级钻取：列 0 展示按团队规模降序排列的业务线，后续列展示直属下级，末列铺满网格
2. 常驻路径条与点击定位：顶栏显示选中节点的完整链路，点击层级项平滑滚动定位至对应列
3. 列滚动位置记忆：按列归属 key 记忆纵向滚动位置，重建前后保持原有位置
4. 全局搜索直接跳转：全局搜索直接跳转到完整路径，自动展开各级分栏
5. 底部字段条：常驻展示当前选中成员的完整属性
