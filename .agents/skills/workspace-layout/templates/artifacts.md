...
本文规定开发过程中产生的文件放在哪里、保留多久、如何收尾。{{BUSINESS_POINTER}}
...
| 层 | 位置 | Git | 内容 | 生命周期 |
|---|---|---|---|---|
| 正式资产 | {{ASSET_PATHS}} | 跟踪 | 源码、测试、正式截图基线、规范、ADR | 随代码评审演进 |
...
- 完整 ID 须匹配 `^[0-9]{8}_[0-9]{6}_[a-z0-9]+(-[a-z0-9]+){1,4}$`{{ID_TOOL_NOTE}}。
...
需要长期生效的结论不停留在工作区：{{PROMOTION_TARGETS}}，可复用脚本迁入{{TOOLS_DIR}}。
...
{{LEGACY_DIRS}} 为旧布局，已迁移完毕，不再新建。工具或 skill 误建这些目录时，按下表迁入 `.work/`，原目录名不合规时按创建时间与主题重新命名：
